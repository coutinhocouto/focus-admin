...
var app_demo = {
    page_like: {
        load: function(){
            ...
            jQuery.get("assets/php/get_likes.php?page="+url,function(data){
                jQuery("#page-like").html("<span class=\"icon-heart\"></span> "+data+" likes");
            });
            
            jQuery("#page-like").on("click",function(){
                jQuery.get("assets/php/like.php?page="+url,function(data){
                    jQuery("#page-like").html("<span class=\"icon-heart\"></span> "+data+" likes");
                });
    ...
    solutions: {
        bank: {
            change_password: function(){
                ...
                jQuery("#show_password").change(function(){
                    ...
                    if(jQuery(this).is(":checked")) {                        
                        jQuery("#old_password, #rep_password, #new_password").attr("type","text");                        
                    }else{                        
                        jQuery("#old_password, #rep_password, #new_password").attr("type","password");                        
                    }
            ...
            change_pin: function(){
                jQuery('#modal-change-pin').on('shown.bs.modal', function(){
                    jQuery("input.mask_pin").mask('99-99');
                    jQuery("input.mask_pin").focus();
                });                
            }
        }
    },    
    googlemap: function(){
        ...
        if(jQuery("#google_world_map").length > 0){
        ...
        if(jQuery("#google_map_markers").length > 0){
        ...
        if(jQuery("#google_map_style").length > 0){
    ...
    jvectormap: function(){
        ...
        if(jQuery("#jvm-world-map").length > 0){
            jQuery("#jvm-world-map").vectorMap({
                map: "world_mill_en",
        ...
        if(jQuery("#jvm-us_map").length > 0){
            jQuery("#jvm-us_map").vectorMap({
                map: "us_aea_en", 
                backgroundColor: "#F5F5F5",
                regionsSelectable: true,
                regionStyle: {
        ...
        if(jQuery("#jvm-world-map-markers").length > 0){
            jQuery('#jvm-world-map-markers').vectorMap({
                map: 'world_mill_en',                
                backgroundColor: "#F5F5F5",
        ...
        if(jQuery("#jvm-us-map-labels").length > 0){
            
            jQuery("#jvm-us-map-labels").vectorMap({            
                map: 'us_aea_en',     
                backgroundColor: "#F5F5F5",
                regionStyle: {
                    selected: {fill: "#4FB5DD"},
    ...
    charts: {
        ...
        rickshaw: function(){
            ...
            if(jQuery("#charts-lineplot").length > 0){
                // line chart
                var sin = [], cos = [], sin2 = [];
                ...
                var rlp_resize = function () {
                    rlp.configure({
                        width: jQuery("#charts-lineplot").width(),
                        height: jQuery("#charts-lineplot").height()
                    });
                    rlp.render();
                }
                ...
                var rlc_resize = function () {
                    rlc.configure({
                        width: jQuery("#charts-lines").width(),
                        height: jQuery("#charts-lines").height()
                    });
                    rlc.render();
                }
                ...
                var rbc_resize = function () {
                    rbc.configure({
                        width: jQuery("#charts-column").width(),
                        height: jQuery("#charts-column").height()
                    });
                    rbc.render();
                }
                ...
                var graph = new Rickshaw.Graph({
                    ...
                    renderer: 'area',
                    width: jQuery("#charts-legend").width(),
                ...
                var resize = function () {
                    graph.configure({
                        width: jQuery("#charts-legend").width(),
                        height: jQuery("#charts-legend").height()
                    });
                    graph.render();
                }
